  ...
  Future<void> _registrarExtintor() async {
    try {
      ...
      final data = {
        ...
        'tipo_id': _selectedTipo, // Usando a variável _selectedTipo
      ...
      final response = await http.post(
        Uri.parse(
            'http://192.168.0.6:3001/registrar_extintor'), // URL do seu servidor
      ...
      if (response.statusCode == 200 && responseData['success']) {
        // Obtém a URL do QR Code do responseData
        final qrCodeUrl = responseData['qrCode'];

        if (qrCodeUrl != null) {
          // Atualize o estado para exibir o QR Code na mesma tela
          setState(() {
            _qrCodeUrl = qrCodeUrl; // Armazena o URL do QR Code
          });
        }
      } else {
